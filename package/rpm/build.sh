...
podman build --build-arg GOLANG_ARCH=${GOLANG_ARCH} --build-arg GOLANG_VERSION=${GOLANG_VERSION} -t configsync_build_rpm:${CONFIGSYNC_VERSION} .
rm -rf rpms
mkdir -p rpms
podman run --user root -v $(readlink -f rpms):/root/rpmbuild/RPMS:Z -e CONFIGSYNC_VERSION=${CONFIGSYNC_VERSION} -e BUILD_DATE="${BUILD_DATE}" -it configsync_build_rpm:${CONFIGSYNC_VERSION}
cp rpms/*/*.rpm .
mv *.rpm ../../../artifacts
cd ../
rm -rf build/
